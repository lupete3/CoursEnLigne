		...
		<div class="row">
	      ...
	      <div class="col-md-12">
	          <h1>Cours disponibles pour vous</h1>
				...
				<?php 
					$query2 = $bd->prepare("SELECT A.idCours,A.designation AS designCours, A.resume FROM cours AS A,enseignant AS B WHERE A.idEns = B.idEnseignant AND B.idEnseignant = ? AND A.statut = ? ");
					$query2->execute(array($idEnseignant,'programme'));
					while ($don2 = $query2->fetch()) { ?>

						<div class="col-md-6">
							<div style="border-radius: 8px 0px 0px 8px" class="border-strong-left border-info p-3 m-3 bg-cream">
								<center>
		                            <h4><a href="coteEtudiants.php?id=<?php echo $don2['idCours'] ?>">Côter ce cours</a></h4>
		                        </center>
